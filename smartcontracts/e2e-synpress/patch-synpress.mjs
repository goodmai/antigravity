...
// Resolve the synpress-metamask playwright bundle (pnpm hoists under .pnpm).
const glob = path.join(__dirname, 'node_modules/.pnpm');
const pkgDir = fs.readdirSync(glob).find(d => d.startsWith('@synthetixio+synpress-metamask@'));
if (!pkgDir) { console.error('synpress-metamask not installed'); process.exit(1); }
...
console.log(`Applied ${n} patch(es) to ${path.relative(__dirname, target)}`);
console.log(n === 0 ? '(all patches already present)' : 'Done.');
