...
### T1 — Scaffold `contracts/` (Hardhat + Foundry)
- **Why:** the repo has JS tooling (`viem`, vitest) but no Solidity project.
- **Implementation:** add `contracts/` with Foundry (`foundry.toml`, `forge` tests) and
  Hardhat (TS deploy scripts via `viem`). Pin `pragma`. Keep it alongside the existing
  academy tooling; do not disturb `package.json` test scripts.
- **Acceptance:** `forge build` and `npx hardhat compile` both pass on an empty contract.
...
### T9 — CI workflow `.github/workflows/contracts.yml`
- **Why:** the existing `static.yml` only deploys Pages; contracts need their own gate.
- **Implementation:** on PR, run `forge build`, `forge test`, coverage, and `hardhat compile`;
  cache deps; report gas snapshot (compare to sandbox constants in Yellowpaper §6).
- **Acceptance:** CI required check passes/fails correctly on PRs.
