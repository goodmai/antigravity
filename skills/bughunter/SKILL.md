...
| **BUG-014** | Chipotle TEE (Rocket) ↔ lit e2e | CI лит-тест падает на шаге `[5/10] Encrypting`: `Lit operation failed: chipotle http 429: <!doctype html> ... <title>429 too many requests</title> ... <small>rocket</small>`. Локально проходит. | HTML-страница 429 с футером `rocket` отдаётся **самой нодой Chipotle** (Rust Rocket), а не внешним прокси. Нода рейт-лимитит / ещё прогревается после `create_wallet`, и `lit_action` (encrypt) получает 429. Клиент [lit-sdk-chipotle.js](file:///home/g/projects/antigravity/smartcontracts/buckets/lit-sdk-chipotle.js) бросал ошибку сразу, без ретрая. 429 = запрос НЕ обработан → ретрай идемпотентен. | В [lit-sdk-chipotle.js](file:///home/g/projects/antigravity/smartcontracts/buckets/lit-sdk-chipotle.js) добавить `fetchWithRetry` (экспон. backoff + jitter, учёт `Retry-After`, ретрай на 429/500/502/503/504 и сетевые сбои) и использовать в `callLitAction`. Обернуть `create_wallet` в [run-e2e-lit-nft.mjs](file:///home/g/projects/antigravity/smartcontracts/e2e/run-e2e-lit-nft.mjs) и [run-e2e-lit.mjs](file:///home/g/projects/antigravity/smartcontracts/e2e/run-e2e-lit.mjs). |
