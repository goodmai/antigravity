...
| `e2e` | custom Node 22 | Drives `run-e2e.mjs`: encrypt → publish to **real** Greenfield testnet (chain 5600) → DRM-wrap the master key (⚠️ historically Lit `datil-dev`, **shut down 2026-02-25** → migrate to **Chipotle/Lit v3**, `api.dev.litprotocol.com`) → register course → Bob purchases → operator re-wraps the envelope on the `CoursePurchased` event → Bob decrypts and asserts plaintext equality → Eve rejected at both layers → pull-payments withdrawn. |
...
| DRM layer (Chipotle / Lit v3) | encrypt is free; decrypt only needs a SIWE personal_sign | **No real BNB** — ⚠️ old Lit `datil*` P2P nets shut down 2026-02-25; use **Chipotle** REST (`api.dev.litprotocol.com`), auth via API key / x402 |
...
2. **DRM-wrap (Chipotle / Lit v3** — old `datil-dev` shut down 2026-02-25**)** —
   master is encrypted under `addressAllowlist(Alice)` initial ACC. The opaque
   ciphertext + ACC land in `manifest.lit`. settlement re-sums exactly:
